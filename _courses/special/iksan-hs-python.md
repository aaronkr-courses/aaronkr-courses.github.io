---
layout: course
init: Maester HS
title: Python Basic Course for Robotics Training
subtitle: 파이썬기초과정 로봇특화교육
description: 2021년 방학 • 국립전북기계공업고등학교
logo: https://res.cloudinary.com/dobann7wi/image/upload/q_auto/f_auto/v1777038667/iksan-gg-logo_zgu6py.png
img: assets/img/books/python-byte.png
importance: 3
category: Special
data_file: 2021/iksan_gg_lectures

grading:
  attendance: 20
  midterm: 30
  final: 30
  homework: 20

Main-Text:
  - text: "주교재"
    author: "Swaroop C. H. 저, 박정빈 역"
    title: <strong>A Byte of Python</strong>
    publisher: "2022년 11월"
    link: "https://drive.google.com/file/d/1gZsVBSQwd1pfPzhwSuoOgJD_gCRtMEq6/view?usp=sharing"
    image: books/python-byte.png
---

국립전북기계공업고등학교 로봇 특화 교육을 위한 파이썬 기초 과목입니다. Python 기초 문법과 로봇 제어를 위한 프로그래밍 역량을 함양합니다.

This introductory Python course was taught to high school students for robotics specialization training, covering Python fundamentals and programming for robot control.
